...
function flt_mission_explore_addon(&$result) {
  /** @see sn_flt_mission_explore_addon */
  /** @see core_festival::expedition_result_adjust(), core_festival::attach_expedition_handlers(), FestivalActivityPuzzleExpedition::fleet_explore_adjust_result() */
  return sn_function_call('flt_mission_explore_addon', array(&$result));
}
 ...
 * @return int
 */
function flt_mission_explore($fleetEvent) {
  ...
  $result              = array(
    ...
    '$userId'              => $fleetEvent->fleet['fleet_owner'],
    '$outcome_list'        => array(),
...
//  switch(FLT_EXPEDITION_OUTCOME_LOST_FLEET) { // TODO DEBUG!
  ...
  if (!empty($fleet) && $fleet_row['fleet_amount']) {
    ...
    $query_data['fleet_mess'] = FLEET_STATUS_RETURNING;
