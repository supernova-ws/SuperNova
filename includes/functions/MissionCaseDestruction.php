...
function MissionCaseDestruction($FleetRow) {
   ...
   if ($FleetRow['fleet_start_time'] <= time()) {
      if ($FleetRow['fleet_mess'] == 0) {
         ...
         $QryMoonData          = "SELECT * FROM {{table}} WHERE ";
         $QryMoonData       .= "`galaxy` = '" . $TargetMoon['galaxy'] . "' AND ";
         $QryMoonData       .= "`system` = '" . $TargetMoon['system'] . "' AND ";
         $QryMoonData       .= "`lunapos` = '" . $TargetMoon['planet'] . "';";
         $QryMoon               = doquery($QryMoonData, 'lunas', true);
         
         $MoonSize = $QryMoon['diameter'];
         $MoonID      = $QryGalaxy['id_luna'];
         $MoonName = $QryMoon['name'];
