  ...
  if ($CurrentPlanet['planet_type'] != 3) { // Is it a moon?
    ...
    foreach($resList as $resName){ // Now, for each type of resource we...
      ...
      if($CurrentPlanet[$resName]<0){ // correction for negative resources if any
        ...
        $debug->warning('Negative Resources','Player have negative resources on '.$CurrentPlanet['galaxy'].':'.$CurrentPlanet['system'].':'.$CurrentPlanet['planet'].'. Compensating '.$CurrentPlanet[$resName].' of '.$resName);
      }
