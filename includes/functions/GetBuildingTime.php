 ...
 * @version 1.1 copyright (c) 2010 Gorlum for http://supernova.ws
   ...
   [%] Fixed bug when low-level lab on planet with lesser PlanetID blocks Intergalactic Tech
   [%] Fixed bug when lab that normally can't paticipate in research join it anywhere with Intergalactic Tech
 * @version 1.0
...
function GetBuildingTime ($user, $planet, $Element)
{
  ...
  elseif (in_array($Element, $reslist['tech']))
  {
    ...
    else
    {
      ...
      $inves = doquery("SELECT SUM(laboratory) as laboratorio FROM {{table}} WHERE id_owner='{$user['id']}' AND laboratory>={$lab_require} order by laboratory asc limit {$limite}", 'planets', true);
