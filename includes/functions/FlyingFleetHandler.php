...
function FlyingFleetHandler (&$planet) {
  ...
  doquery('LOCK TABLE {{table}}aks WRITE, {{table}}rw WRITE, {{table}}errors WRITE, {{table}}messages WRITE, {{table}}fleets WRITE, {{table}}planets WRITE, {{table}}users WRITE, {{table}}logs WRITE, {{table}}iraks WRITE');

  COE_missileCalculate();
  /*
  $fleets = array();
  $_fleets = doquery("SELECT *, fleet_start_time AS fleet_time FROM {{fleets}} WHERE `fleet_start_time` <= '".$time_now."';");
  while ($row = mysql_fetch_array($_fleets)) $fleets[] = $row;

  $_fleets = doquery("SELECT *, fleet_end_time AS fleet_time FROM {{fleets}} WHERE `fleet_end_time` <= '".$time_now."';");
  while ($row = mysql_fetch_array($_fleets)) $fleets[] = $row;
