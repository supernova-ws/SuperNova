...
if ($mode == 2) {
  if ($user['rpg_points'] >= $darkmater_cost) {
    ...
    if ( in_array($Selected, $reslist['officier']) ) {
      ...
      if ( $Result == 1 ) {
        ...
        rpg_pointsAdd($user['id'], -($darkmater_cost), "Spent for officer {$lang['ttle'][$Selected]} ID {$Selected}");
  ...
  $parse['alv_points']   = $user['rpg_points'];
  $parse['disp_off_tbl'] = "";
  for ( $Officier = 601; $Officier <= 615; $Officier++ ) {
    ...
    if ( $Result != 0 ) {
      ...
      $bloc['off_lvl']      = $user[$resource[$Officier]];
