...
class V2UnitModel extends \Entity\KeyedModel {
  ...
  public function __construct(\Common\GlobalContainer $gc) {
    ...
    $this->accessors->set(P_CONTAINER_SET, 'snId', array($this, 'setSnId'));
    $this->accessors->set(P_CONTAINER_UNSET, 'snId', array($this, 'unsetSnId'));

    // This crap code is until php 5.4+. There we can use $this binding for lambdas
    $propertyName = 'timeStart';
    $this->accessors->set(P_CONTAINER_IMPORT, $propertyName, array($gc->types, 'dateTimeImport'));
    $this->accessors->set(P_CONTAINER_EXPORT, $propertyName, array($gc->types, 'dateTimeExport'));
