...
//      msg_send_simple_message($this->src_user['id'], '', $this->fleet['fleet_start_time'], MSG_TYPE_SPY, $lang['sys_mess_qg'], $lang['sys_mess_spy_report'], $this->spy_message);
  ...
  protected function doSpying() {
    ...
    $this->spy_detected = $this->missionReport->isSpyDetected();
    $this->spy_probes = $this->missionReport->getProbesNumber();
  }
...
//    );
//
//    $spy_message = "<table class=\"no_border_image\" width=\"440\" cellspacing = \"1\"><tr><td class=\"c\" colspan=\"4\">{$lang['sys_spy_materials']} {$target_planet_row['name']} ";
...
//      $this->coe_compress_add_units($this->general->getGroupsByName('fleet'), $target_planet_row, $combat_pack[0]);
//    }
//    if ($spy_diff >= 3) {
//      $spy_message .= "<div class='spy_medium'>" . $this->flt_spy_scan($target_planet_row, 'defense', $lang['tech'][UNIT_DEFENCE], $target_user_row) . "</div>";
//      $this->coe_compress_add_units($this->general->getGroupsByName('defense_active'), $target_planet_row, $combat_pack[0]);
//    }
//    if ($spy_diff >= 5) {
...
//
//    $target_unit_list = 0;
//    foreach ($this->general->getGroupsByName('fleet') as $unit_id) {
