  ...
  case 'write':
    ...
    if(sys_get_param_str('msg_send')) {
      ...
      if(empty($error_list)) {
        ...
        DBStaticMessages::msg_send_simple_message($recipient_id, $user['id'], SN_TIME_NOW, MSG_TYPE_PLAYER, "{$user_safe_name} [{$user['galaxy']}:{$user['system']}:{$user['planet']}]", $subject, $text, true);
