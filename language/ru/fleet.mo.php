...
$lang = array_merge($lang, array(
  ...
  'fl_expe_notech' => '<li>Перед отправкой экспелиции нужно исследовать экспедиционную технологию!</li>',
  'fl_expe_max' => '<li>Вы не можете отправить ещё одну экспедицию. Развивайте экспедиционную технологию.</li>',
  ...
  'fl_page5_hint' => '<ul>  <li>Отметьте галочками в строчках колоний виды ресурсов, которые хотите свезти на текущую планету  <li>Галочка в заголовке окна позволяет поставить или снять отметки для указанного типа ресурсов сразу на всех колониях  <li>Галочки в столбце ВСЕГО позволяют отметить или снять отметки для всех ресурсов на указанной колонии  <li>Галочки в столбце ВСЕГО предназначены исключительно для облегчения работы с интерфейсом и <span class="negative">НЕ ВЛИЯЮТ</span> на набор свозимых ресурсов  <li>В перевозке ресурсов участвуют только транспортные корабли: Малый транспорт, Большой транспорт и Супертранспорт  <li>Корабли загружаются в порядке убывания емкости трюма</ul>',
  'fl_err_no_ships' => 'Во флоте нет ни одного корабля. Вернитесь на предыдущую страницу и выберите корабля для отправки флота',
  'fl_shrtcup' => array(
    PT_PLANET => '(П)',
    PT_DEBRIS => '(О)',
    PT_MOON => '(Л)',
