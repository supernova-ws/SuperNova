...
$lang = array_merge($lang, array(
  ...
  'gate_use_gate'   => 'Использовать ворота',
  'gate_ship_sel'   => 'Выделить корабли',
  'gate_ship_dispo' => 'доступно',
  'gate_jump_btn'   => 'Выполнить прыжок!!',
  'gate_jump_done'  => 'ворота находятся в стадии перезарядки!<br>ворота будут готовы к использованию через: ',
  'gate_wait_dest'  => 'Точка назначения ВРАТ в стадии подготовки!ворота будут готовы к использованию через: ',
  'gate_no_dest_g'  => 'На конечной точке назначения не обнаруженно ворот для перемещения флота',
  'gate_wait_star'  => 'ворота находятся в стадии перезарядки!<br>ворота будут готовы к использованию через: ',
  ...
  'info' => array(
    ...
    15 => array(
      ...
      'description_short' => 'Единственным оборудованиеем этой фабрики являются наносборщики - специализированные комплексы, позволяющие конструировать объекты из отдельных молекул и атомов. Каждый уровень нанофабрики повышает скорость любого строительства в два раза.'
    ),
    ...
    22 => array(
      'description' => 'Огромное хранилище для добытых руд. Чем оно больше, тем больше металла можно в нём хранить. Если оно заполнено, то добыча металла прекращается.',
      'description_short' => 'Хранилище для необработанных металлов до их дальнейшей переработки.'
    ),

    23 => array(
      'description' => 'В этом огромном хранилище складируется ещё не обработанный кристалл. Чем хранилище больше, тем больше кристалла там можно хранить. Если оно заполнено, то добыча сего ресурса прекращается.',
      'description_short' => 'Хранилище кристаллов до их дальнейшей переработки.'
    ),

    24 => array(
      'description' => 'Огромные ёмкости для хранения добытого дейтерия. Они обычно находятся вблизи космических портов. Чем они больше, тем больше дейтерия в них может сберегаться. Если они заполнены, то добыча дейтерия прекращается.',
      'description_short' => 'Огромные ёмкости для сберегания добытого дейтерия.'
    ),
    ...
    44 => array(
      'description' => 'Ракетные шахты служат для хранения ракет. С каждым уровнем можно хранить на пять межпланетных или десять ракет-перехватчиков больше. Одна межпланетная ракета требует места в два раза больше, чем ракета-перехватчик. Возможно любое комбинирование различных типов ракет.',
    ...
    113 => array(
      ...
      'description_short' => 'Исследование энергетических технологий позволяет улучшить отдачу энергостанций.'
    ),
    ...
    115 => array(
      'description' => 'Реактивный двигатель основывается на принципе отдачи. Материя, разогретая до высоких температур, выбрасывается в направлении, противоположном движению и даёт ускорение кораблю. Эффективность этих двигателей достаточно мала, но они достаточно надёжны, дёшевы в производстве и обслуживании. Кроме того они занимают гораздо меньше места на корабле по сравнению с остальными двигателями, поэтому их всё ещё достаточно часто можно встретить на маленьких кораблях. Так как реактивные двигатели являются основой любого полёта в космос, следует исследовать их как можно раньше. Дальнейшее развитие этих двигателей делает следующие корабли с каждым уровнем на 10% быстрее: малые транспорты (до тех пор пока не будет исследован импульсный двигатель 5-го уровня) , большие транспорты, лёгкие истребители, переработчики и шпионские зонды.',
      'description_short' => 'Дальнейшее развитие этих двигателей делает некоторые корабли быстрее, однако каждый уровень повышает скорость лишь на 10%.'
    ),

    117 => array(
      'description' => 'Импульсный двигатель основывается на принципе отдачи, причём разогрев материи осуществляется в ходе ядерной реакции. Также можно впрыснуть дополнительную массу. Дальнейшее развитие этих двигателей делает следующие корабли с каждым уровнем на 20% быстрее: малый транспорт, бомбардировщики(до тех пор пока не будет исследован гиперпространственный двигатель 8-го уровня), крейсеры, тяжёлые истребители и колонизаторы. Каждый уровень развития увеличивает радиус действия межпланетных ракет.',
      'description_short' => 'Импульсный двигатель основывается на принципе отдачи. Дальнейшее развитие этих двигателей делает некоторые корабли быстрее, однако с каждым уровнем повышает скорость лишь на 20%.'
    ),
    ...
    118 => array(
      ...
      'description_short' => 'Благодаря пространственно-временному изгибу в непосредственном окружении корабля пространство сжимается, чем быстрее преодолеваются далёкие расстояния. Чем выше развит гиперпростраственный привод, тем выше сжатие пространства, благодаря чему с каждым уровнем скорость кораблей повышается на 30%.'
    ),
    ...
    202 => array(
      ...
      'description_short' => 'Малый транспорт - это манёвренный корабль, который может быстро транспортировать сырьё на другие планеты. После исследования импульсного двигателя 5го уровня, корабли переоснащаются.'
    ),
    ...
    203 => array(
      ...
      'description_short' => 'Дальнейшее развитие малых транспортов позволило создать корабли, обладающие большей вместительностью и, благодаря более развитому двигателю, способными передвигаться быстрее, чем малый транспорт, до тех пор, пока на малых транспортах не устанавливаются импульсные двигатели 5-го уровня'
    ),

    201 => array(
      'description' => 'Последнее слово в технологиях транспортировки. Супертранспорт - гигантский транспортный корабль, оснащенный импульсными двигателями. Его скорость невелика, а расход топлива весьма большой, однако это полностью окупается экстраординарной вместительностью. Супертранспорт оснащен лишь лазерными противометеоритными туррелями, но имеет мощный щит.',
      'description_short' => 'Гигантская самоходная баржа, оснащенная импульсными двигателями. Оборудован мощным щитом, но из оружия имеет только лазерные противометеоритные туррели.'
    ),
    ...
    206 => array(
      'description' => 'С развитием тяжёлых лазеров и ионных пушек тяжёлые истребители всё больше вытеснялись. Несмотря на многочисленные усовершенствования огневая мощь и бронирование не могли быть настолько изменены, чтобы действенно противостоять этим оборонительным орудиям. Поэтому было решено построить новый класс кораблей, который объединял бы в себе больше бронирования и огневой мощи. Так появился крейсер. Крейсеры почти втрое сильней защищены, чем тяжёлые истребители и обладают более чем удвоенной огневой мощью. К тому же они очень быстры. Нет лучшего оружия против средней защиты. Почти столетие крейсеры неограниченно господствовали во вселенной. С появлением орудий Гаусса и плазменных орудий их господство закончилось. Однако и сегодня их охотно применяют против групп истребителей.',
      'description_short' => 'Крейсеры почти втрое сильней защищены, чем тяжёлые истребители, а по огневой мощи они превосходят тяжёлые истребители почти в два раза. К тому же они очень быстры.'
    ),
    ...
    207 => array(
      ...
      'description_short' => 'Линкоры как правило составляют основу флота. Их тяжёлые орудия, высокая скорость и большой грузовой тоннаж делают их серьёзными противниками.'
    ),

    208 => array(
      'description' => "Этот хорошо защищённый корабль служит покорению новых планет, что необходимо развивающейся империи. Он используется в новой колонии в качестве поставщика сырья - его разбирают и используют весь полезный материал для освоения нового света. Каждая империя может иметь максимум {$config->player_max_colonies} колоний (не считая главную планету).",
    ...
    216 => array(
      'description' => 'Вам предоставляется вознаграждение со стороны императора за ваши навыки жестокости.',
      'description_short' => "{$lang['tech'][216]} - флагман космического флота Империи. Огромная стоимость постройки с лихвой компенсируется ужасающей огневой мощью и продвинутой защитой. Один корабль такого класса способен в одиночку разгромить средний флот."
    ),
    ...
    MRC_GEOLOGIST => array(
      'description' => 'Геолог - это признанный эксперт в астроминералогии и кристаллографии. Со своей командой металлургов и химиков он поддерживает межпланетные правительства при разработке новых источников ресурсов и оптимизации их очистки.',
      'description_short' => 'к добыче металла, кристаллов и дейтрия за каждый уровень.'
    ),

    MRC_POWERMAN => array(
      'description' => 'Специалист по управлению энергией. В мирное время он повышает уровень энергетических сетей на колониях.',
      'description_short' => 'к производству энергии за каждый уровень.'
    ),

    MRC_CONSTRUCTOR => array(
      'description' => 'Конструкторы - это гениальные учёные, и их всегда можно найти там, где заканчивается грань технически возможного. Их код никогда не сможет разгадать ни один нормальный человек, и одним своим присутствием они вдохновляют учёных империи.',
      'description_short' => 'к скорости постройки кораблей за каждый уровень.'
    ),

    MRC_ARCHITECT => array(
      'description' => 'Архитектор - современнейший строитель Империи. Его ДНК была подвергнута мутациям, что обеспечило ему экстраординарный склад ума и сверхчеловеческую силу. Архитектор в одиночку может спроектировать и построить целый город.',
      'description_short' => 'к скорости постройки зданий за каждый уровень.'
    ),
    ...
    MRC_ACADEMIC => array(
      ...
      'description_short' => 'к скорости исследований за каждый уровень.'
    ),

    MRC_FORTIFIER => array(
      'description' => 'Фортификатор - армейский инженер-специалист. Его глубокие познания в области оборонительных систем позволяют сократить сроки строения систем обороны Империи.',
      'description_short' => 'к скорости постройки защитных сооружений и ракет за каждый уровень.'
    ),
    ...
    MRC_STOCKMAN => array(
      ...
      'description_short' => 'к размеру складов за каждый уровень.'
    ),
    ...
    MRC_SPY => array(
      ...
      'description_short' => 'к уровню шпионажа за каждый уровень.'
    ),
    ...
    MRC_ADMIRAL => array(
      ...
      'description_short' => 'к броне, щитам и атаке кораблей за каждый уровень.'
    ),
    ...
    MRC_COORDINATOR => array(
      ...
      'description_short' => 'дополнительный флот за каждый уровень.'
    ),
    ...
    MRC_NAVIGATOR => array(
      ...
      'description_short' => 'к скорости кораблей за каждый уровень.'
    ),

    MRC_DEFENDER => array(
      'description' => 'Количество работ, проведенных Вами по укреплению Империи, даёт Вам шанс построить Планетарную защиту. Планетарная защита - ультрасовременное решение во всех технологических отраслях Империи.',
      'description_short' => 'Позволяет строить Планетарную защиту.'
    ),
    ...
    MRC_DESTRUCTOR => array(
      ...
      'description_short' => "Позволяет строить на верфи корабль {$lang['info'][214]['name']}."
    ),

    MRC_ASSASIN => array(
      'description' => "Ассасин - доверенный убийца Императора. Но это не единственное его качество. Вместе с Разрушителем, Ассасин разработал {$lang['tech'][216]}. Центральный компьютер корабля настроен на ДНК Ассасина. Поэтому он - единственный человек, способный управлять этим кораблем",
      'description_short' => "Возможность строить {$lang['info'][216]['name']} на верфи"
    ),
    ...
    MRC_EMPEROR => array(
      ...
      'description_short' => 'Позволяет изменить характеристики Императора'
    )
  )
));
