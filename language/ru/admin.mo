...
$lang = array_merge($lang, array(
  ...
  'adm_opt_ver_check_hint' => 'Проверка версии выполняется в ручном режиме по нажатию кнопки "Проверить версию". При этом передаются только анонимные данные - текущая версия БД, номер релиза и версия игры',
  ...
  'adm_opt_ver_check_auto_hint' => 'Вы можете включить автоматическую проверку версии игры. При этом передаются те же самые данные, что и в случае ручной проверки, однака сама проверка выполняется автоматически раз в заданный период времени (по умолчанию - раз в сутки). Подробнее см. документацию',
  ...
  'adm_opt_ver_response' => array(
    ...
    SNC_VER_ERROR_SERVER => 'Ошибка сервера обновлений. Срочно уведомите разработчика!',
