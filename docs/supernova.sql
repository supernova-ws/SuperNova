...
CREATE TABLE `sn_counter` (
  ...
  KEY `I_counter_user_id` (`user_id`,`device_id`,`browser_id`,`user_ip`,`user_proxy`),
  KEY `I_counter_visit_time` (`visit_time`,`counter_id`),
...
CREATE TABLE `sn_festival_highspot` (
  ...
  `params` text COLLATE utf8_unicode_ci NOT NULL COMMENT 'Параметры хайспота в виде JSON-encoded',
  PRIMARY KEY (`id`),
...
CREATE TABLE `sn_festival_ois_player` (
  ...
  `ois_count` int(10) unsigned DEFAULT NULL COMMENT 'OiS player controlled last tick',
...
INSERT INTO `sn_config` VALUES ('db_version', '43');
