...
class V2PropertyContainerTest extends PHPUnit_Framework_TestCase {
  ...
  public function testAssignAccessor() {
    $this->object->assignAccessor('test', P_CONTAINER_GETTER_PIMPLE, null);
    $this->assertAttributeEquals(array(), 'accessors', $this->object);

    $lambda = function(){};
    $this->object->assignAccessor('test', P_CONTAINER_GETTER_PIMPLE, $lambda);
    $this->assertAttributeEquals(array('test' => array(P_CONTAINER_GETTER_PIMPLE => $lambda)), 'accessors', $this->object);

    $this->setExpectedException('Exception', 'Error assigning callable in V2PropertyContainer! Callable typed [' . P_CONTAINER_GETTER_PIMPLE . '] is not a callable or not accessible in the scope');
    $this->object->assignAccessor('test', P_CONTAINER_GETTER_PIMPLE, 1);
  }
  ...
  public function test__set() {
    ...
    $that = $this->object;
    // Installing setter
    $this->object->assignAccessor('p1', P_CONTAINER_SETTER, function($value) use ($that) {$that->setDirect('p2', $value . '3');});
    $this->object->p1 = 'v2';
    // Setting value
    ...
    $this->object->assignAccessor('p2', P_CONTAINER_GETTER, function() use ($that) {return $that->p3 . '4';});
  ...
  public function testClearProperties() {
    ...
    $that = $this->object;
    $this->object->assignAccessor('p3', P_CONTAINER_SETTER, function($value) use ($that) {$that->setDirect('p4', $value);});
    $this->object->assignAccessor('p3', P_CONTAINER_UNSETTER, function() use ($that) {unset($that->p4);});
  ...
  public function test__isset() {
    ...
    $that = $this->object;
    $this->object->assignAccessor('p2', P_CONTAINER_GETTER, function() use ($that) {return $that->p3;});
    $this->object->p4 = function ($c) {return $c->p5;};
    $this->object->assignAccessor('p6', P_CONTAINER_GETTER, function() use ($that) {return $that->p7;});
