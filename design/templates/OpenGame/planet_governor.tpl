<!-- IF PLANET_GOVERNOR_ID || $PLANET_GOVERNOR_ID -->
  <!-- IF ! $PLANET_GOVERNOR_ID -->
  ...
  <div class="unit_preview" style="" id="unit{production.ID}" unit_id="{production.ID}">
    <div onclick="document.location='overview.php?mode=manage&cp={$PLANET_ID}'">
      <span style="position: absolute; left: 0px; top: 0px; width: 100%; height: 100%">
        <img style="border-style: none;" src="{dpath}gebaeude/{$PLANET_GOVERNOR_ID}.jpg" height="100%" width="100%">
      </span>

      <span style="position: absolute; bottom: 0px; left: 0px; width: 100%; height: 3ex; font-size: 100%;" class="icon_alpha">
        <!-- IF $PLANET_GOVERNOR_NAME -->{$PLANET_GOVERNOR_NAME}<!-- ELSE -->{PLANET_GOVERNOR_NAME}<!-- ENDIF -->
      </span>

      <!-- IF PLANET_GOVERNOR_CAN_BUILD -->
      ...
      <span style="position: absolute; top: 0; left: 20%; width: 60%; height: 16px; text-align: center; font-size: 120%;" class="icon_alpha">
        <!-- IF PLANET_GOVERNOR_LEVEL -->{PLANET_GOVERNOR_LEVEL}<!-- ENDIF --><!-- IF $PLANET_GOVERNOR_LEVEL -->{$PLANET_GOVERNOR_LEVEL}<!-- ENDIF --><!-- IF PLANET_GOVERNOR_LEVEL_MAX -->/{PLANET_GOVERNOR_LEVEL_MAX}<!-- ENDIF --><!-- IF $PLANET_GOVERNOR_LEVEL_MAX -->/{$PLANET_GOVERNOR_LEVEL_MAX}<!-- ENDIF -->
      </span>
    ...
    <span style="position: absolute; bottom: 0px; right: 0px;" class="icon_alpha" onclick="document.location='infos.php?gid={$PLANET_GOVERNOR_ID}'">
      <div class="icons icon-info"></div>
    </span>
  </div>
<!-- ELSE -->
  <a href="overview.php?mode=manage&cp={$PLANET_ID}">{L_sys_no_governor}</a>
<!-- ENDIF -->
