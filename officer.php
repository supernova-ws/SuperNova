...
if($mercenary_id = sys_get_param_int('mercenary_id'))
{
  try
  {
    ...
    if($mercenary_level < 1 || $mercenary_level > $sn_data[$mercenary_id])
    {
      throw new Exception($lang['mrc_msg_error_wrong_level'], ERR_ERROR);
    }

    if(!array_key_exists($mercenary_period = sys_get_param_int('mercenary_period'), $mrc_hire_discount))
    {
    ...
    $darkmater_cost = ceil($darkmater_cost[BUILD_CREATE][RES_DARK_MATTER] * $mercenary_period * $mrc_hire_discount[$mercenary_period] / $config->empire_mercenary_base_period);
...
//  if($Result = ($config->empire_mercenary_temporary || mrc_officer_accessible ( $user, $mercenary_id )))
  {
    ...
    $template->assign_block_vars('officer', array(
      ...
      'LEVEL'       => $mercenary_level, // $user[$sn_data[$mercenary_id]['name']]
